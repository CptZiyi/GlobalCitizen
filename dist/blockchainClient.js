...
(function (BlockChainModule) {
    class BlockchainClient {
        ...
        async createProjectPledge(args) {
        ...
        async sendPledgeToGlobalCitizen(args) {
            // Calling transaction1 transaction in the smart contract
            let response = await args.contract.submitTransaction(args.function, args.pledgeId);
            return response;
        }
        async sendPledgeToGovOrg(args) {
            // Calling transaction1 transaction in the smart contract
            let response = await args.contract.submitTransaction(args.function, args.pledgeId);
            return response;
        }
        async updatePledge(args) {
            // Calling transaction1 transaction in the smart contract
        ...
        async transferFunds(args) {
            // Calling transaction1 transaction in the smart contract
            let response = await args.contract.submitTransaction(args.function, args.pledgeId);
            return response;
        }
        async queryProject(args) {
            // Calling transaction1 transaction in the smart contract
